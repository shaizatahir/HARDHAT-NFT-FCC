  ...
  : describe("Dynamic SVG NFT Unit Tests", function () {
      ...
      describe("mintNft", () => {
        it("emits an event and creates the NFT", async () => {
          ...
          await expect(dynamicSvgNft.mint(highValue)).to.emit(
            dynamicSvgNft,
            "CreatedNFT"
          );
          ...
          // console.log("This Is TokenURI ----", highTokenUri);
      ...
      describe("svgToImageUri", () => {
        it("converts svg to token uri", async () => {
          const highsvg = fs.readFileSync("./images/dynamicNft/happy.svg", {
            encoding: "utf-8",
          });
          const tx = await dynamicSvgNft.svgToImageURI(highsvg);
          assert.equal(highSVGimageUri, tx);
        });
      });
    });
